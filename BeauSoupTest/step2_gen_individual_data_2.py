...
if __name__ == '__main__':
    ...
    cursor.execute(query)
    main_results = cursor.fetchall()
    for i_result in range(173715,len(main_results)):
        main_result = main_results[i_result]
        print "Processing " + str(i_result) + " out of " + str(len(main_results))
        tic = main_result[0]
        date = main_result[1]
        ...
        query = 'INSERT INTO edgar_8k_form4_individual (tic, date,event_date,filed_8k,lag,filename,contains_pr,' \
                ...
                'edgar_8k_form4_individual where tic=%s GROUP BY PERSONID'

        cursor.execute(query, (tic,date,event_date,filed_8k,lag,filename,contains_pr,tic))
        db.commit()