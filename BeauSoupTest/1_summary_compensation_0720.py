...
########################################################################
####README!!!!
####There are two main functions, the get_table and rearrange_date
#### These functions call other functions to work for them.
#### get_table extracts cells from the html table
#### rearrange_date identify the columns and insert them into the database.
########################################################################
########################################################################
...
def adjust_col_strings(raw, rowspan):
    ...
    for st in raw:
        ...
        processed = st.replace(u'\u2014', '-').replace(u'\x97', '-').replace(u'\u0096', '-').replace(u'\uFE58',
                                                                                                '-').encode('ascii',
...
def clean_data(data):
    ...
    # print_row(data)
    data[1:] = remove_empty_data_cols(data[1:])
    # print_row(data)
...
def classify_rows(data, pointer=0):
    ...
    for i, row in enumerate(data[pointer:]):
        if is_data_row(row):
            ...
            while k <= i:
                # print 'k is ', k, data[i-k]
                if 'name' in data[i-k][0].lower() or 'position' in data[i-k][0].lower() or any(['salary' in e.lower() for e in data[i-k]]):
                    e = 1
                    # print e
                    while data[i-e][0] and e is not k:
                        e+=1
                        # print 'e is ', e
                    # print 'i-e is ', i-e, data[i-e]
                    return i-e
                elif k+1>i:
                    break
                k+=1
                # elif 'name' in data[i-k][0].lower() or 'position' in data[i-k][0].lower():
                #     return i - 1
...
def split_line(data):
    ...
    if '-' in orig_year:
        ...
        orig_year = orig_year.split()
    if any([len(split_year(row[1]))> 1 for row in data]):
        # print_row(data)
        lines = []
        # print orig_year
        # lines.append()
        data = [[re.sub(u'\|', ' ', cell) for cell in row] for row in data]
        # print_row(data)
        for i, row in enumerate(data):
            if is_data_row(row):
                # print row
...
def get_table(filename):
    ...
    # print data
    # data = data.text.encode('utf-8').decode('ascii', '-')
    # print data
    # data = data.replace(u'\u2014', '-').replace(u'\x97', '-').replace(u'\u0096', ' ').replace(u'\uFE58',
    #                                                                                             '-').encode('ascii',
    #                                                                                                 'replace').replace('?', ' ').replace('\n', ' ')
...
if __name__ == '__main__':
    txtpath = ''
    with open(txtpath + 'remainder_0720.txt', 'w') as f:
        f.write("+++++++ \n")
    with open(txtpath + 'error_file_0720.txt', 'w') as tf:
        tf.write("++++++ \n")
    # with open('test.csv', 'w') as cf:
    #     writer = csv.writer(cf)
    #     writer.writerow(['test'])

    # # #Test for 1 file on server
    # filename = '/local/bill_test/exec_comp_tables_final/'
    # filename = '/Users/bill/PycharmProjects/BeauSoupTest/companies/MSFT/'
    # accession_cik = '0001193125-03-051346_789019_1'
    ...
    # table = rearrange_data(data, csvname, 'remainder_0720.txt')
    # print_row(table)
    ...
    #TEST 1 FILE LOCALLY
    # txtpath = ''
    # filename = '/Users/bill/PycharmProjects/BeauSoupTest/companies/MSFT/0001193125-03-051346_789019_1.txt'
    # # for filename in glob.glob(path):
    # #     # print filename
    # #     csvname = os.path.basename(filename)
    # #     csvname = os.path.splitext(csvname)[0]
    # #     accession = csvname.split('_')[0]
    # textname = os.path.basename(filename)
    # print "textname " + textname
    # csvname = os.path.splitext(textname)[0]
    ...
    #     # print_row(data)
    #     try:
    #         table = rearrange_data(data, csvname, txtpath+'remainder_0720.txt')
    #         # writeToCSV(table, 'test.csv', 'a')
    #         # print_row(table)
    #
    #     except Exception, e:
    ...
    #         #print data
    #         with open(txtpath + 'error_file_0720.txt', 'a') as tf:
    ...
    sql = '''CREATE TABLE IF NOT EXISTS bill_exec_0720(
    ...
    table2 = '''CREATE TABLE IF NOT EXISTS bill_name_0720(
                        id BIGINT NOT NULL AUTO_INCREMENT,
                        name_title TEXT,
                        accession VARCHAR(255),
                        cik INT(11),
    ...
    for filename in glob.glob(path):
        ...
        try:
            ...
            try:
                ...
                query = "INSERT INTO bill_exec_0720(`name`, `year`, `title`, `salary`,`bonus`,`stock_awards`," \
                        "`option_awards`, `RSUs`, " \
                    "`nonequity_incentive_plan_compensation`,`change_in_pension_value_and_nonqualified_deferred_compensation`,  " \
                    "`all_other`,  `securities_underlying_options`," \
                    "`remainder`, `total`," \
                ...
                query2 = "INSERT INTO bill_name_0720(`name_title`, `cik`, `accession`, `file_date` )"\
                    "VALUES(%s, %s, %s, %s)"
